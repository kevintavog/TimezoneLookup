...
public class RangicLoggerFilter: HTTPRequestFilter, HTTPResponseFilter {
	...
	public func filterHeaders(response: HTTPResponse, callback: (HTTPResponseFilterResult) -> ()) {
		...
		var extra = ""
		if let serviceErrorMessage = response.request.scratchPad[RangicLoggerFilter.serviceErrorMessageKey] {
			extra += "serviceError=\"\(serviceErrorMessage)\""
		}

		for (key, value) in response.request.scratchPad {
			if key != "start" && key != RangicLoggerFilter.serviceErrorMessageKey && !key.starts(with: "_") {
				extra += " \(key)=\"\(value)\""
			}
		}
